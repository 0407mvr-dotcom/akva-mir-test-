...
// ============================================
// СЛАЙДЕРЫ В КАТАЛОГЕ (ГОРИЗОНТАЛЬНАЯ ПРОКРУТКА)
// ============================================
...
function initCatalogSliders() {
    ...
    navButtons.forEach(function(btn) {
        btn.addEventListener('click', function(e) {
            ...
            const isPrev = this.classList.contains('slider-nav-btn--prev');
...
// ============================================
// ИЗМЕНЯЕМ ФУНКЦИЮ РЕНДЕРИНГА ДЛЯ СЛАЙДЕРОВ
// ============================================

const originalRenderCatalogProducts = window.renderCatalogProducts;

window.renderCatalogProducts = function(containerId, products) {
    const container = document.getElementById(containerId);
    if (!container) return;
    
    const isSlider = container.classList.contains('catalog-slider-container');
    ...
    if (isSlider) {
        ...
        if (typeof originalRenderCatalogProducts === 'function') {
            originalRenderCatalogProducts(containerId, products);
        }
    }
};

// ============================================
// ОБНОВЛЯЕМ РЕНДЕРИНГ ДЛЯ СТРАНИЦЫ КАТАЛОГА
// ============================================

const originalRenderAllProducts = window.renderAllProducts;

window.renderAllProducts = function() {
    const currentPage = window.location.pathname.split('/').pop().toLowerCase();
    
    if (currentPage === 'catalog.html' || currentPage === '') {
        // Хиты продаж — теперь тоже слайдер
        const hitSlider = document.getElementById('hitSlider');
        if (hitSlider) {
            window.renderCatalogProducts('hitSlider', hitProducts);
        }
        
        const waterSlider = document.getElementById('waterSlider');
        if (waterSlider) {
            window.renderCatalogProducts('waterSlider', waterProducts);
        }
        
        const equipmentSlider = document.getElementById('equipmentSlider');
        if (equipmentSlider) {
            window.renderCatalogProducts('equipmentSlider', equipmentProducts);
        }
        
        const accessorySlider = document.getElementById('accessorySlider');
        if (accessorySlider) {
            window.renderCatalogProducts('accessorySlider', accessoryProducts);
        }
        
        const giftSlider = document.getElementById('giftSlider');
        if (giftSlider) {
            window.renderCatalogProducts('giftSlider', giftCardProducts);
        }
    } else {
        if (typeof originalRenderAllProducts === 'function') {
            originalRenderAllProducts();
        }
    }
};
...
setTimeout(function() {
    if (window.location.pathname.includes('catalog.html')) {
        ...
        setTimeout(initCatalogSliders, 300);
    }
}, 200);
// ============================================
// ДУБЛИРУЮЩЕЕ МЕНЮ ПРИ СКРОЛЛЕ
// ============================================
(function() {
    const navDuplicate = document.getElementById('pageNavDuplicate');
    if (!navDuplicate) return;
    
    let isVisible = false;
    const offset = 200; // через сколько пикселей показывать меню
    
    window.addEventListener('scroll', function() {
        const scrollY = window.pageYOffset || document.documentElement.scrollTop;
        
        if (scrollY > offset && !isVisible) {
            navDuplicate.classList.add('visible');
            isVisible = true;
        } else if (scrollY <= offset && isVisible) {
            navDuplicate.classList.remove('visible');
            isVisible = false;
        }
    });
})();

// ===== ПРОГРЕСС-БАР ДЛЯ СЛАЙДЕРОВ (МОБИЛЬНАЯ ВЕРСИЯ) =====
function initSliderProgress() {
    const sliders = document.querySelectorAll('.catalog-slider-container');
    
    sliders.forEach(function(slider) {
        const sliderId = slider.id;
        const progressFill = document.getElementById('progressFill_' + sliderId);
        ...
        function updateProgress() {
            ...
            if (maxScroll > 0) {
                ...
                // Минимальный прогресс 8% (чтобы полоска всегда была видна)
                const finalPercent = Math.max(percent, 8);
        ...
        // Первоначальное обновление с задержкой (ждём рендеринга)
        setTimeout(updateProgress, 300);
    });
}

// Запускаем после рендеринга слайдеров
setTimeout(function() {
    if (window.location.pathname.includes('catalog.html')) {
        setTimeout(initSliderProgress, 500);
    }
}, 300);
});