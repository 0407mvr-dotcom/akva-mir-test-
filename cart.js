...
function renderCart() {
    ...
    // ===== БЛОК СДАЧИ БУТЫЛОК (С ВЫБОРОМ КОЛИЧЕСТВА) =====
    // Считаем количество бутылок в корзине
    var totalBottles = 0;
    cart.forEach(function(item) {
        if ((item.id >= 1 && item.id <= 7) || (item.id >= 21 && item.id <= 25)) {
            totalBottles += item.quantity;
        }
    });
    
    var bottleWord = 'бутылок';
    if (totalBottles === 1) bottleWord = 'бутылка';
    else if (totalBottles > 1 && totalBottles < 5) bottleWord = 'бутылки';
    
    html += `
        <div class="cart-bottle-return">
            <div class="cart-bottle-return__header">
                <input type="checkbox" id="return-bottle-toggle" class="custom-checkbox">
                <label for="return-bottle-toggle">Хочу сдать бутылки</label>
            </div>
            <div class="cart-bottle-return__body" id="bottleReturnBody" style="display: none;">
                <p class="cart-bottle-return__info">В вашей корзине <strong>${totalBottles}</strong> ${bottleWord}. Сколько хотите сдать?</p>
                <div class="cart-bottle-return__controls">
                    <button class="bottle-return-btn bottle-return-btn--minus" onclick="changeReturnQuantity(-1)">−</button>
                    <input type="number" class="bottle-return-input" id="returnQuantity" value="0" min="0" max="${totalBottles}">
                    <button class="bottle-return-btn bottle-return-btn--plus" onclick="changeReturnQuantity(1)">+</button>
                </div>
                <p class="cart-bottle-return__hint">Вы можете сдать только те бутылки, которые находятся в корзине</p>
            </div>
        </div>
    `;
    ...
    updateCartTotal();
    updateDeliveryBlock();
    updateBottleReturnMax();
    
    // ===== ПЕРЕКЛЮЧАТЕЛЬ ДЛЯ БЛОКА СДАЧИ =====
    var toggle = document.getElementById('return-bottle-toggle');
    if (toggle) {
        // Удаляем старый обработчик, чтобы не дублировался
        toggle.removeEventListener('change', toggle._listener);
        toggle._listener = function() {
            var body = document.getElementById('bottleReturnBody');
            if (body) {
                body.style.display = this.checked ? 'block' : 'none';
            }
            if (!this.checked) {
                var input = document.getElementById('returnQuantity');
                if (input) input.value = '0';
            }
        };
        toggle.addEventListener('change', toggle._listener);
    }
}

// ===== УПРАВЛЕНИЕ СДАЧЕЙ БУТЫЛОК =====

// Изменение количества
window.changeReturnQuantity = function(delta) {
    var input = document.getElementById('returnQuantity');
    if (!input) return;
...
// Обновляем максимальное количество при изменении корзины
window.updateBottleReturnMax = function() {
    var input = document.getElementById('returnQuantity');
    if (!input) return;
    ...
    input.max = totalBottles;
    if (parseInt(input.value) > totalBottles) {
        input.value = totalBottles;
    }
    
    // Обновляем подпись
    var info = document.querySelector('.cart-bottle-return__info');
    if (info) {
        var word = 'бутылок';
        if (totalBottles === 1) word = 'бутылка';
        else if (totalBottles > 1 && totalBottles < 5) word = 'бутылки';
        info.innerHTML = 'В вашей корзине <strong>' + totalBottles + '</strong> ' + word + '. Сколько хотите сдать?';
    }
};
...
window.changeReturnQuantity = changeReturnQuantity;
window.updateBottleReturnMax = updateBottleReturnMax;
